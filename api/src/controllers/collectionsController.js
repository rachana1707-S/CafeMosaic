...
const collectionsController = {
  ...
  getUserCollections: async (req, res) => {
    try {
      ...
      // Transform data to match frontend expectations with real visit counts
      const transformedCollections = await Promise.all(
        collections.map(async (collection) => {
          // Get actual visit counts from the visits table
          const visitCounts = await Promise.all(
            collection.coffeeShops.map(async (item) => {
          ...
          const actualVisitedCount = visitCounts.reduce((sum, count) => sum + count, 0);
          ...
          return {
            ...
            dateModified: collection.updatedAt,
            placesCount: collection.coffeeShops.length,
            visitedCount: actualVisitedCount, // Real count from visits table
            places: collection.coffeeShops.map((item, index) => ({
              id: item.coffeeShop.id.toString(),
              placeId: item.coffeeShop.placeId,
              name: item.coffeeShop.name,
              address: item.coffeeShop.address,
              ...
              distance: null,
              notes: item.notes,
              isVisited: visitCounts[index] > 0, // Real visit status
              visitedDate: null, // Will be populated separately
              dateAdded: item.createdAt,
              personalRating: null,
              tags: null,
              openingHours: item.coffeeShop.openingHours,
  ...
  getCollectionById: async (req, res) => {
    try {
      ...
      // Get visit status for each place if user is the owner
      let placesWithVisitStatus = [];
      if (canEdit) {
        placesWithVisitStatus = await Promise.all(
          collection.coffeeShops.map(async (item) => {
            const visit = await prisma.coffeeShopVisit.findFirst({
              where: {
                userId: req.user.id,
            ...
            return {
              ...
              distance: null,
              notes: item.notes,
              isVisited: !!visit, // Real visit status from database
              visitedDate: visit?.visitDate || null,
              dateAdded: item.createdAt,
              personalRating: null,
              tags: null,
              openingHours: item.coffeeShop.openingHours,
        ...
        // For public collections, don't show visit status or notes
        placesWithVisitStatus = collection.coffeeShops.map(item => ({
          id: item.coffeeShop.id.toString(),
          placeId: item.coffeeShop.placeId,
          name: item.coffeeShop.name,
          address: item.coffeeShop.address,
          ...
          longitude: item.coffeeShop.longitude,
          distance: null,
          notes: '', // Hide notes for public collections
          isVisited: false, // Hide visit status for public collections
          visitedDate: null,
          dateAdded: item.createdAt,
          personalRating: null,
          tags: null,
      ...
      const visitedCount = canEdit ? placesWithVisitStatus.filter(p => p.isVisited).length : 0;
      ...
      const transformedCollection = {
        ...
        dateModified: collection.updatedAt,
        placesCount: collection.coffeeShops.length,
        visitedCount: visitedCount, // Real count from visits table
        canEdit: canEdit,
        creator: {
          username: collection.user.username,
          name: collection.user.name
        },
        places: placesWithVisitStatus
      };
  ...
  // Add a place to a collection (UPDATED - with visit integration)
  addPlaceToCollection: async (req, res) => {
    try {
      const collectionId = parseInt(req.params.collectionId);
      const { 
  ...
  // Batch add places to multiple collections
  batchAddPlaceToCollections: async (req, res) => {
    try {
      const {
        collectionIds,
  ...
  // Update a place in a collection (notes only - visit status handled by visits controller)
  updatePlaceInCollection: async (req, res) => {
    try {
      const collectionId = parseInt(req.params.collectionId);
      const placeId = req.params.placeId;
      const { notes } = req.body; // Only handle notes now, visits handled separately
  ...
  getPublicCollections: async (req, res) => {
    try {
      ...
      const transformedCollections = collections.map(collection => ({
        ...
        color: collection.color || '#FFD700',
        placesCount: collection.coffeeShops.length,
        visitedCount: 0, // Default for public collections
        dateCreated: collection.createdAt,
        creator: {
          username: collection.user.username,
          name: collection.user.name
