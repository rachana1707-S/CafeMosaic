...
export default function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (JSON.stringify(formData.preferences) !== JSON.stringify(originalData.preferences)) {
        updatedFields.preferences = formData.preferences;
      }
      ...
      if (response.ok) {
        ...
        setOriginalData(newUserData); // Update original data
        setIsEditing(false);
        ...
        if (response.status === 400 && errorMessage.includes('username')) {
